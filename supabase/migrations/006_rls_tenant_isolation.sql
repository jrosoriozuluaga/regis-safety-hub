...
COMMENT ON FUNCTION auth.current_empresa_id() IS
...
CREATE OR REPLACE FUNCTION auth.is_regis_admin()
RETURNS BOOLEAN
LANGUAGE sql
SECURITY DEFINER
STABLE
...
COMMENT ON FUNCTION auth.is_regis_admin() IS
...
CREATE OR REPLACE FUNCTION auth.is_regis_staff()
RETURNS BOOLEAN
LANGUAGE sql
SECURITY DEFINER
STABLE
...
COMMENT ON FUNCTION auth.is_regis_staff() IS
...
-- =========================================================================
-- Pattern:
--   SELECT/INSERT/UPDATE: auth.is_regis_staff() OR empresa_id = auth.current_empresa_id()
--   DELETE: auth.is_regis_admin() only (clients don't delete)
...
CREATE POLICY "tenant_select" ON public.trabajadores
  ...
  USING (
    auth.is_regis_staff()
    OR empresa_id = auth.current_empresa_id()
  );
  ...
  USING (
    ...
    OR EXISTS (
      ...
      WHERE mr.id = riesgos_matriz.matriz_id
        AND mr.empresa_id = auth.current_empresa_id()
    )
  );
  ...
  USING (
    ...
    OR EXISTS (
      ...
      WHERE c.id = actas_comite.comite_id
        AND c.empresa_id = auth.current_empresa_id()
    )
  );
  ...
  USING (
    ...
    OR EXISTS (
      ...
      WHERE pe.id = amenazas_vulnerabilidad.plan_id
        AND pe.empresa_id = auth.current_empresa_id()
    )
  );
  ...
  USING (
    ...
    OR EXISTS (
      ...
      WHERE em.id = recomendaciones_medicas.examen_id
        AND em.empresa_id = auth.current_empresa_id()
    )
  );
  ...
  USING (
    ...
    OR EXISTS (
      ...
      WHERE ce.id = items_cumplimiento.cumplimiento_id
        AND ce.empresa_id = auth.current_empresa_id()
    )
  );
...
CREATE POLICY "config_modify" ON public.configuracion_sistema
  ...
  USING (auth.is_regis_admin())
  WITH CHECK (auth.is_regis_admin());
